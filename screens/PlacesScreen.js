...
export default function PlacesScreen() {
  ...
  const handleSubmit = async () => {
    ...
    const firstCity = responseJson.features[0];
    const newPlace={
      ...
      const newPlaceToPost = {nickname:user.nickname, ...newPlace}
      fetch('https://locapic-back-ashen.vercel.app/places',{
        method:"POST",
        headers:{'Content-Type': 'application/json'},
        body:JSON.stringify(newPlaceToPost)
      }).then(res=>res.json())
  ...
  const deleteCity = (cityName) =>{
    ...
    const bodyData = {name: cityName, nickname:user.nickname}
    fetch('https://locapic-back-ashen.vercel.app/places',{
        method:"DELETE",
        headers:{'Content-Type': 'application/json'},
  ...
  const places = user.places.map((data, i) => {
    return (
      <View key={i} style={styles.card}>
        ...
        <FontAwesome name='trash-o' onPress={() => deleteCity(data.name)} size={25} color='#ec6e5b' />
      </View>
    );
  });
