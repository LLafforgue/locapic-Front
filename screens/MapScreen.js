...
import { addPlace, addManyPlaces, deleteAll } from '../reducers/user';
...
export default function MapScreen() {
  ...
  const load = <Text style={{flex:1, backgroundColor:"rgba(255, 255, 255, 0.5)"}}>Loading</Text>
  ...
  useEffect(() => {
    ...
    fetch(`https://locapic-back-ashen.vercel.app/places/${user.nickname}`)
    .then(res=>res.json())
    .then(data=>{
      // console.log(data);
      if (data.result){
      dispatch(addManyPlaces(data.places));
      }else{
        dispatch(deleteAll());
      }
  ...
  const handleNewPlace = () => {
    ...
    setNewPlace('');
      ...
      setNewPlace( newPlace + `#${user.places.filter(el=>el.name===newPlace).length+1}`)
    }
  };
